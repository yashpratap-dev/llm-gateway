...
/**
 * {@link LLMProvider} implementation for the Groq cloud inference platform.
 *
 * <p>Groq offers ultra-low latency completions via custom LPU hardware and exposes an
 * OpenAI-compatible REST API. This provider uses a reactive {@link WebClient} for both
 * synchronous (blocking) and streaming completion requests.</p>
 *
 * <p>The bean is only registered when {@code providers.groq.api-key} is present in the
 * environment, so the application starts cleanly if only OpenAI is configured.</p>
 *
...
public class GroqProvider implements LLMProvider {
     ...
     *
     * @param webClient    pre-configured Groq {@link WebClient} (base URL + auth header)
     * @param properties   Groq-specific configuration (model, timeout)
     * @param objectMapper Jackson mapper for deserialising stream chunks
     */
    public GroqProvider(@Qualifier("groqWebClient") WebClient webClient,
     ...
     * @throws ProviderUnavailableException if Groq returns a 5xx response
     * @throws ProviderTimeoutException     if no response arrives within the configured timeout
     * @throws ProviderException            if Groq returns a 4xx error or an empty choices list
     */
    @Override
        ...
        ProviderChatResponse response = webClient.post()
                ...
                .onStatus(
                        ...
                        resp -> resp.bodyToMono(String.class)
                                .defaultIfEmpty("")
                                .map(body -> new ProviderException("Groq client error: " + body)))
                .onStatus(
                        status -> status.is5xxServerError(),
                        resp -> resp.bodyToMono(String.class)
                                .defaultIfEmpty("")
                                .map(body -> new ProviderUnavailableException("GROQ", new RuntimeException(body))))
                .bodyToMono(ProviderChatResponse.class)
                .timeout(Duration.ofSeconds(properties.timeoutSeconds()),
                        Mono.error(new ProviderTimeoutException("GROQ")))
                .block();
    ...
    /**
     * Opens a Server-Sent Events stream to the Groq API and returns a reactive stream of chunks.
     *
        ...
        return webClient.post()
                ...
                .flatMap(line -> {
                    try {
                        ...
                        return Flux.just(new ChatChunk(chunk.id(), ProviderName.GROQ.name(), delta, done));
                    } catch (Exception e) {
                        log.debug("Skipping malformed stream chunk from Groq: {}", e.getMessage());
                        return Flux.empty();
                    }
                });
    }
    ...
    private ChatResponse mapToChatResponse(ProviderChatResponse response) {
        if (response.choices() == null || response.choices().isEmpty()) {
            throw new ProviderException("Groq returned empty choices in response");
        }
        ...
        Usage usage = new Usage(
                ...
                0.0);
        GatewayMeta meta = new GatewayMeta(ProviderName.GROQ.name(), false, 0L);
        return new ChatResponse(response.id(), response.model(), ProviderName.GROQ.name(), choices, usage, meta);
    }
}
